...
async function run() {
  console.log('Connecting to existing Vite dev server on http://localhost:5173...');
  ...
  try {
    ...
    await wait(1500);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'v2_welcome_desktop.png') });
    await page.close();
    await context.close();
    ...
    await page.goto('http://localhost:5173/?v=2');
    await wait(1500);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'v2_welcome_mobile.png') });
    ...
    await wait(1500);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'v2_quiz_mobile.png') });
    await page.close();
    await context.close();

    // 4. Desktop Results Page - Overview (1280x1200)
    ...
    await wait(2000);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'v2_results_desktop.png') });
    ...
    await wait(1000);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'v2_results_strengths_tab_desktop.png') });
    await page.close();
    await context.close();
    ...
    await page.goto('http://localhost:5173/?v=2&fp=75&sb=35&bs=60&ay=40&dv=80');
    await wait(2000);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'v2_results_mobile.png') });
    await page.close();
    await context.close();
    ...
    await wait(1500);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'v2_gallery_page_desktop.png') });
    ...
    await wait(1000);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'v2_archetype_details_modal_desktop.png') });
    await page.close();
    await context.close();
    ...
    await wait(1500);
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'v2_methodology_page_desktop.png') });
    await page.close();
    await context.close();
